/*  The simplest way to implement the Behavioral Pattern: Commit Reveal
    Reference from: Floersch, Karl, https://gist.github.com/karlfloersch (12/04/2018)
...
contract Election {
    ...
    function ParticipantsElection(uint _commitPhaseLengthInSeconds, string _choice1, string _choice2) public {
    ...
    function commitVote(bytes32 _voteCommit) public {
        if (now > commitPhaseEndTime) revert(); // Only allow commits during committing period
        
        // Check if this commit has been used before
        bytes memory bytesVoteCommit = bytes(voteStatuses[_voteCommit]);
        if (bytesVoteCommit.length != 0) revert();
    ...
    function revealVote(string _vote, bytes32 _voteCommit) public {
        if (now < commitPhaseEndTime) revert(); // Only reveal votes after committing period is over
    ...
    function getWinner () constant returns(string) {
        ...
        if (votesForChoice1 + votesForChoice2 != voteCommits.length) revert();
